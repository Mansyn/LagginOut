...
export class EditorArticleComponent implements OnInit, OnDestroy {
    ...
    this.sub = this.route.params.subscribe(params => {
      ...
      if (this.id == 0) {
        ...
        this.form = this.fb.group({
          title: [this.article.title || null, Validators.compose([Validators.maxLength(50), Validators.required])],
  ...
  saveArticle() {
    if (this.form.valid) {
      ...
      if (this.id == 0) {
        ...
        this.articlesService.updateArticle(this.id.toString(), article)
          .then((data) => {
            this.openSnackBar('Article Saved', 'OKAY');
            this._location.back();
          })
